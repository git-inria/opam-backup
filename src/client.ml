...
let proceed_todelete t nv =
  ...
  let err = Dirname.exec (Path.C.lib_dir t.compiler)  "proceed_todelete" [remove] in
  if err <> 0 then
...
(*  let commands_s = List.map (fun cmd -> String.concat " " cmd)  commands in *)
  let err = Dirname.exec
    ~add_to_path:[Path.C.bin t.compiler] p_build
    (Printf.sprintf "Build [%s]" (N.to_string (NV.name nv)))
    commands in
  if err = 0 then
